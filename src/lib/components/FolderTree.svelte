...
{#snippet entry(row: TreeRow, kind: EntryKind)}
  ...
  <span class="grid size-4 shrink-0 place-items-center">
    ...
    {#if row.expandable && kind === 'dir'}
      <button
        ...
        class="text-muted-foreground hover:text-foreground grid size-4 place-items-center"
        onclick={(event) => {
  ...
  {#if node.status === 'unchanged' && node.has_changes}
    <span class="bg-mod-ink size-1.5 shrink-0 rounded-full opacity-80" aria-hidden="true"></span>
  {/if}
{/snippet}

{#snippet warning()}
  <TriangleAlertIcon class="text-unknown-ink size-3.5 shrink-0" aria-label="Could not be read" />
{/snippet}
...
{#snippet columns()}
  <div
    class="bg-card sticky top-0 z-10 flex shrink-0 items-center border-b text-xs"
...
{#if result.identical}
  <div class="flex h-full flex-col">
    {@render columns()}
    <div class="text-muted-foreground grid min-h-0 flex-1 place-items-center text-center">
      <div class="space-y-1">
        <p class="text-foreground text-sm">The two folders are identical</p>
        <p class="text-xs">
...
{:else}
  <div
    ...
    aria-activedescendant={selectedIndex >= firstVisible && selectedIndex < lastVisible
      ...
      : undefined}
    tabindex="0"
    class="focus-visible:ring-ring/50 h-full overflow-x-hidden overflow-y-auto text-[13px] outline-none focus-visible:ring-2 focus-visible:ring-inset"
  >
    {@render columns()}
        ...
        {#each visible as row, offset (row.node.path)}
          ...
          <div
            ...
            class="flex items-stretch whitespace-nowrap select-none {selected
              ? 'bg-accent ring-brand/60 ring-1 ring-inset'
          ...
          >
            ...
            {:else}
              ...
              <div class="flex min-w-0 flex-1 items-center gap-1.5 pr-3" style:padding-left={indent}>
                ...
                {#if change}
                  <span class="text-muted-foreground shrink-0 text-[11px]">{change}</span>
                {/if}
